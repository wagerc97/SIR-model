...
def librarySolver(solverChoice, t_0, ndays, t, S_0, I_0, R_0, beta, gamma):
    """
    Solve coupled system of ODEs.
    :param solverChoice: lsoda or 'RK45'
    :param S_0, I_0, R_0: initial conditions
    :return: matrix containing the solution vectors for all 3 ODEs
    """
    if solverChoice == 1:
        print("Solver: odeint()")
        #"Solve a system of ordinary differential equations using lsoda from the FORTRAN library odepack."
        solution = scipy.integrate.odeint(
            func=SIR_model,       # system of ODEs
            y0=[S_0, I_0, R_0],   # initial condition
            t=t,                  # time points
            args=(beta, gamma)    # rates
        )
    elif solverChoice == 2:
        print("Solver: solve_ivp()")
        solution = scipy.integrate.solve_ivp(
            fun=lambda t, y: SIR_model(y, t, beta, gamma),  # system of ODEs
            y0=[S_0, I_0, R_0],     # initial condition
            t_span=[t_0, ndays],    # time points
            t_eval=t,               # Times at which to store the computed solution
            method='RK45',  # default
...
if __name__ == '__main__':
    ...
    # solverChoice = int(solverChoice)
    solverChoice = 2
    solution = np.array(librarySolver(solverChoice, t_0, ndays, t, S_0, I_0, R_0, beta, gamma))
